
export const CARD_BG = [
   { id: 1, bgcolor: 'linear-gradient(135deg, #e6f8f3 0%, #f7fcfa 100%)' },
   { id: 2, bgcolor: 'linear-gradient(135deg, #fef9e7 0%, #fffdf4 100%)' },
   { id: 3, bgcolor: 'linear-gradient(135deg, #eaf7ff 0%, #f3fbff 100%)' },
   { id: 4, bgcolor: 'linear-gradient(135deg, #fff2e9 0%, #fff8f3 100%)' },
   { id: 5, bgcolor: 'linear-gradient(135deg, #e7f6fe 0%, #f4fafd 100%)' },
   { id: 6, bgcolor: 'linear-gradient(135deg, #f5f5f5 0%, #fbfbfb 100%)' },
   { id: 7, bgcolor: 'linear-gradient(135deg, #fff4fc 0%, #fff8fd 100%)' },
   { id: 8, bgcolor: 'linear-gradient(135deg, #e8fef3 0%, #f5fef8 100%)' },
   { id: 9, bgcolor: 'linear-gradient(135deg, #f0ecff 0%, #f7f5ff 100%)' },
   { id: 10, bgcolor: 'linear-gradient(135deg, #fef2f2 0%, #fff8f8 100%)' },
];

export const APP_FEATURES = [
    {
        id: "01",
        title: "Tailored For Each Role",
        description:
        "Get interview questions and model answers based on each specific role, level, and skills"
    },
    {
       id: "02",
       title: "Learn On Your Own Terms",
       description:
       "Expand answers only when you're ready. Dive deeper into any concept instantly with AI-powered detailed explanations"
    },
    {
        id: "03",
        title: "Capture Your Input",
        description:
        "Add custom notes to each session and pin important questions to the top, making your learning more organized"
     },
     {
        id: "04",
        title: 'Understand the Reasoning Behind Answers',
        description:
        "Beyond just answers, unlock thorough concept explanations generated by AI, helping you truly grasp each topic"
     },
     {
        id: "05",
        title: "Save, Organize, and Revisit",
        description:
        "Save your interview sets, organize them neatly into your dashboard, and pick up your preperation exactly where you left off"
     },
];